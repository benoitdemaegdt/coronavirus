...
(async() => {
  // 1 - process Santé Publique France dataset
  await processSpfData();
  // 2 - process politologue dataset
  await processPolitologueData();
})()
.catch(err => console.log('err :', err));

async function processPolitologueData() {
  const URL = 'https://www.data.gouv.fr/fr/datasets/r/a7596877-d7c3-4da6-99c1-2f52d418e881';
  const downloadLink = await getDownloadLink(URL);
  await downloadFile(downloadLink, './politologue.json');
  const rawPolitologue = JSON.parse(fs.readFileSync(path.resolve(__dirname, './politologue.json')));

  const COUNTRY_LIST = ['Allemagne', 'Espagne', 'France', 'Royaume-Uni', 'Italie', 'États-Unis'];
  const outputWorld = []

  for (const item of rawPolitologue.PaysData) {
    if (!COUNTRY_LIST.includes(item.Pays)) continue;
    const itemDate = formatDate(item.Date.substring(0,10));
    if (outputWorld.length === 0 || outputWorld[outputWorld.length - 1].date !== itemDate) {
      outputWorld.push({
        date: itemDate,
        countries: [
          {
            country: item.Pays,
            infections: item.Infection,
            deces: item.Deces,
          }
        ]
      });
    } else {
      outputWorld[outputWorld.length - 1].countries.push({
        country: item.Pays,
        infections: item.Infection,
        deces: item.Deces,
      })
  ...
  let jsonWorld = JSON.stringify(outputWorld.reverse().slice(30), null, 2);
  fs.writeFileSync(path.resolve(__dirname, './politologue/countries.json'), jsonWorld);
  fs.unlinkSync(path.resolve(__dirname, './politologue.json'));
}

async function processSpfData() {
  try {
    const outputFrance = [];
    const outputRegions = [];
    ...
    // delete raw file to save space in build
    fs.unlinkSync(path.resolve(__dirname, './spf.csv'));
  } catch (err) {
    console.log('err :', err);
  }
