  ...
  <v-container>
    ...
    <v-row>
      <v-col cols='12' sm='6'>
        <!-- hospitalisations -->
        <v-card>
          <v-card-title>Nombre de patients hospitalisés en {{ getRegionName() }}</v-card-title>
          <v-card-text>
            <line-chart
      ...
      <v-col cols='12' sm='6'>
        <!-- réanimations -->
        <v-card>
          <v-card-title>Nombre de personnes en réanimation en {{ getRegionName() }}</v-card-title>
          <v-card-text>
            <line-chart
    ...
    <v-row>
      <v-col cols='12'>
        <!-- décès -->
        <v-card>
          <v-card-title>Nombre cumulé de personnes décédées en milieu hospitalier en {{ getRegionName() }}</v-card-title>
          <v-card-text>
            <line-chart
...
export default {
  ...
  methods: {
    getLastFigure(category) {
      const lastDataForRegion = regions[regions.length - 1].regions.find(region => region.name === this.getRegionName());
      return lastDataForRegion[category].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ');
    },
    handleRouteChange() {
